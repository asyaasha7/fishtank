...
function GameTransactionCube({ character, position, speed, scale, sphereRef, onCollected }) {
  ...
  useFrame((state, delta) => {
    if (meshRef.current && !isCollected) {
      // Move from right to left (slower speed) - use independent position tracking
      currentPosition.current[0] -= speed * delta * 15
    ...
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      <div style={{
        ...
        <div style={{ 
          ...
          <div style={{ marginBottom: '0.5rem' }}>
            💥 <strong>Avoid Scam Token Hunter cubes</strong> (red)
          </div>
          <div style={{ marginBottom: '0.5rem' }}>
            🎮 Move mouse to fly
          </div>
